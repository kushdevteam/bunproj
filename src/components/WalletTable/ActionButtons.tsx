...
export const ActionButtons: React.FC<ActionButtonsProps> = ({ 
  ...
  const handleAction = useCallback(async (action: 'buy' | 'sell' | 'fund') => {
    if (!isUnlocked) {
      setActionState(prev => ({ ...prev, error: 'Session must be unlocked' }));
      return;
    }
  ...
  const getButtonClass = useCallback((action: string) => {
    ...
    const disabledClass = disabled || !isUnlocked ? 'disabled' : '';
  ...
  const isButtonDisabled = useCallback((action: string) => {
    return disabled || !isUnlocked || (actionState.isLoading && actionState.lastAction !== action);
  }, [disabled, isUnlocked, actionState.isLoading, actionState.lastAction]);
    ...
    <div className="action-buttons">
      <div className="button-group">
        <button
          ...
          title={!isUnlocked ? 'Unlock session to perform actions' : 'Buy tokens with this wallet'}
        >
        ...
        <button
          ...
          title={!isUnlocked ? 'Unlock session to perform actions' : 'Sell tokens from this wallet'}
        >
        ...
        <button
          ...
          title={!isUnlocked ? 'Unlock session to perform actions' : 'Fund this wallet with BNB'}
        >
